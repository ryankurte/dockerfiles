...
# Patched for sentry.io support per https://github.com/espressif/esp-idf/pull/7540
...
def json_corefile():  # type: () -> Optional[list[str]]
    ...
    dump = {
        ...
        "timestamp": args.timestamp if args.timestamp else datetime.datetime.now(datetime.timezone.utc).isoformat(),
        "platform": "c",
        "logger": "espcoredump.py",
        "release": args.release if args.release else os.path.basename(args.prog),
        "tags": {},
    ...
    if extra_note:
        ...
        if marker == ESPCoreDumpElfFile.CURR_TASK_MARKER:
            ...
            # Add task to fingerprint
            dump["fingerprint"].append(task_name)
...
if __name__ == '__main__':
    ...
    json_coredump.add_argument('--release',
                               help='Set release for event')
